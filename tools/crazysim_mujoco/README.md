...
### Canonical oLGMD obstacle detector

The canonical obstacle head is the repository-owned oLGMD1 path. It does not
load FrontNet or ONNX. The stable public runner alias is olgmd-obstacle; the
longer gate_frontnet_olgmd_obstacle_only name remains only as a transport ABI
compatibility identifier.

The controller trusts looming only while forward speed exceeds 0.15 m/s,
requires two distinct positive camera frames, and commands the cache-free 10 m/s^2
TinyMPC emergency-braking formulation. After stopping, it holds for 3.0 seconds, then
resumes with a 2.0-second refractory period. Missing or stale threat packets
remain fail-closed while moving.

A reproducible hallway validation uses the textured square obstacle and walls.
The obstacle moves clear after the first stationary stop:

~~~sh
tools/crazysim_mujoco/run.sh \
  --trajectory straight_20m --duration 18 --launch-prespin 1 \
  --vision-model olgmd-obstacle \
  --vision-scene obstacle \
  --out apps/controller_tinympc_eigen/sim_runs/crazysim/olgmd_hallway_clear_resume \
  --overwrite --video
~~~

The bundle at models/olgmd_obstacle_v1/bundle.json pins the 80x80 oLGMD
contract, 30 Hz cadence, T_spike=0.7, and controller safety parameters.

### Gate FrontNet + oLGMD bring-up

The combined adapter sends the binary oLGMD packet before the four-corner gate
packet and enables the dedicated TinyMPC arbitration mode. It defaults to
`scenes/vision_gate.xml`, whose visible gate is the checksum-pinned four-piece
NewBeeDrone asset:

```sh
tools/crazysim_mujoco/run.sh \
  --trajectory straight --duration 12 --launch-prespin 1 \
  --vision-model gate-frontnet-olgmd --stop-on-contact \
  --out apps/controller_tinympc_eigen/sim_runs/crazysim/gate_olgmd \
  --overwrite
```

This alias currently uses the recovered 160x96 integer gate model and is for
bring-up only. The recorded corner labels use a 0.4826 m inner span, while the
shared NewBeeDrone simulator ABI uses a 0.45 m clear opening and a 0.555 m
rail-center span. That corner-definition mismatch must be measured and resolved
before this path can be flight-qualified.

### IMAV22 official complexity modes

`--imav22-mode gates|static|dynamic` selects the three challenge categories
and automatically loads the IMAV22 arena. The run manifest records the
official environment factor: 1, 5, or 10 respectively. Gates mode removes all
non-gate challenge objects from rendering and collision. Static mode keeps
the published ten-object Isaac inventory: two gates, two panels, two feather
banners, and four substantial 0.30 m diameter challenge poles. The super-thin
standalone pole variant remains excluded. Dynamic mode cycles one of the
eight non-gate objects per event. Position and yaw are interpolated toward
deterministic targets generated by the Isaac seed-22 relocation sampler. A
relocation is delayed unless the complete path clears the drone by the base
clearance plus the obstacle footprint radius. Every attempt/completion is
recorded in `imav22_events.csv`.

```sh
tools/crazysim_mujoco/run.sh \
  --trajectory circle --duration 300 --camera-only \
  --imav22-mode dynamic --random-seed 1 \
  --out apps/controller_tinympc_eigen/sim_runs/crazysim/imav22_dynamic_seed1
```

The production defaults attempt relocation every 30 seconds, require 1.5 m
base clearance, and interpolate over 2 seconds. The corresponding command-line
options exist for short smoke tests and controlled robustness experiments.

The scene mirrors the accepted Isaac Lab
`imav22_nanocopter_visual_fidelity_v3` artifact: exact seed-22 poses and
collision dimensions, the UV-preserving official gate mesh, pinned official
turf/pole/panel/Cyberzoo textures, the traffic rug and curtain/net perimeter,
and intentionally no ceiling. Asset hashes, the upstream commit, derived-asset
labels, and the MIT notice are recorded in
`scenes/textures/imav22/PROVENANCE.md`. The fixed coordinates are a reproducible
reconstruction because the final competition coordinates were not published.

Add `--imav22-object-set extended` to static or dynamic mode for an explicitly
non-official robustness arena. It adds a banded blue barrel, strapped wooden
crate, striped curtain, hazard beam, and paired bollards. The extras use
distinct procedural textures; official objects and venue surfaces use vendored,
checksum-pinned IMAV simulator textures. The official object set remains the
default and is the only set allowed with gates-only mode.
