...
MASS_KG = 0.04338
GRAVITY_MPS2 = 9.81
INERTIA_KGM2 = np.diag([2.5e-5, 2.8e-5, 4.9e-5])
...
def _absolute_derivative(absolute: np.ndarray, motor_command_n: np.ndarray) -> np.ndarray:
    ...
    acceleration = (
        rotation @ np.asarray([0.0, 0.0, force_moment[0] / MASS_KG])
...
def generate_artifact(trajectory_csv: Path, output: Path, rho: float = 250.0) -> dict[str, Any]:
    ...
    if (float(np.min(command_reference)) < -1.0e-6 or
            float(np.max(command_reference)) >
                MAX_MOTOR_THRUST_N + 1.0e-6):
        raise ValueError(
            "trajectory is not actuator-feasible for the stored motor time constant: "
            f"nominal commands span {command_reference.min():.4f}.."
            f"{command_reference.max():.4f} N, limit is "
            f"{MAX_MOTOR_THRUST_N:.4f} N"
        )
