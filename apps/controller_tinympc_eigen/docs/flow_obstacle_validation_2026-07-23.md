...
- `tools/hardware/obstacle_corpus.json` defines 90 positive and 48 negative
...
`tools/hardware/{build_logs,build_artifacts}`. SHA-256 hashes for these
...
`tools/hardware/logs/f27-stationary-threshold-0p004.csv` contains 1,200 rows:
...
`tools/hardware/logs/f27-protected-hover-positive-001.csv`. During the peer
...
`tools/hardware/audit_validation_bundle.py` verifies current commit/dirty state,
