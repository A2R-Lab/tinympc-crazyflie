...
static void UpdateHorizonReference(const setpoint_t *setpoint)
{
  if (enable_traj)
  {
    ...
    // XZ PLANE: Reference is a straight line at constant z
    // Solver must figure out to go up to avoid closing arms
    for (int i = 0; i < NHORIZON; ++i) {
      float t = base_t + i * dt;
      float x = (t < travel_time) ? (traj_speed * t) : traj_dist;
      params.Xref(0, i) = x;
      params.Xref(1, i) = 0.0f;
      params.Xref(2, i) = traj_height;  // Constant z - solver decides avoidance
    }
