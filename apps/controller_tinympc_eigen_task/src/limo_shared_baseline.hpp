...
// Frozen entry (authority w=0, Qz multiplier=1) from the canonical
// authority_cache_bank_16x4_f32 generated by safe-reachability. Keeping
// this single entry separate prevents nominal/CBF/posthoc images from
// carrying the complete 16x4 adaptive LIMO bank.
namespace limo_shared_baseline {
...
static_assert(sizeof(kCacheData) / sizeof(kCacheData[0]) == kCacheScalars,
              "Unexpected shared LIMO baseline cache size");
static_assert(kCacheData[0] == 5.0f,
              "Shared LIMO baseline cache must use rho=5");
